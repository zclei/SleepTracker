...
[spdb,h] = Vplot_speed(refPos,vFR,binSize,ppm);
hold on;subtitle(FN); 
if exist([outDir,FN,'_Sleep.png'],'file')
        saveas (h, [outDir,FN,'_speed?1?.png']);
    else
        saveas (h, [outDir,FN,'_speed.png']);
end
...
%     hold on;subtitle(FN);
%     if exist([outDir,FN,'_Sleep.png'],'file')
%         saveas (h, [outDir,FN,'_distance.png']);
%     else
...
% clear h;
%
VwriteXls(spdb,{'speed','(mm/s)'},outDir,PN,FN,arenaType,binSize);
